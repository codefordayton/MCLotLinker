$("document").ready(function() {
  ...
  $("#addressInput").typeahead({
    ...
    templates: {
      empty: '<div class="empty-message">No eligible properties were found. <br /><span class="error-text">If you provided a complete address, then the property is not eligible at this time.</span></div>'
    }
  });
  ...
  function lookupValue(value) {
    ...
    if (val.length > 2) {
      for (var i = 0; i < allMarkerLength; i++) {
        if ((allMarkers[i].parcelid && allMarkers[i].parcelid.indexOf(val) >= 0) ||
            (allMarkers[i].address && allMarkers[i].address.indexOf(val) >= 0)) {
    ...
    else if (val.length < 2 && markerList.length !== allMarkerLength) {
      console.log('abcd')
      markerList = [];
    }

...
with open('reapitems.csv') as csvfile:
    reader = csv.DictReader(csvfile, ['parcel', 'street', 'eligible', 'paymentplan', 'lastyear', 'paymentwindow', 'class'])
    for row in reader:
        if row['eligible'] != 'Sold' and row['paymentplan'] == 'False' and row['paymentwindow'] == 'False' and row['lastyear'] < '2013' and row['class'] == 'R':
            try:
                ...
                values.append({'parcelid': row['parcel'], 'street': row['street'] + ', ' + get_city(latlon), 'lat': latlon[0], 'lon': latlon[1]})
